...
/// own words: *"the Action has an entry but was stored without it — this can happen
/// when you receive gossip of just an action"*), was dropped without a trace.
///
/// Caught by CI on the v0.7.0 branch as `gold_badge_issued_with_seven_validators`,
/// `left: 6, right: 7`. Both consequences are permanent, because a HarmonyRecord is
/// immutable: participation is understated forever, and the count feeds
...
// only fails when a record happens to arrive without its entry. These cost
// milliseconds and fail every time.
...
mod tests {
    ...
    fn record_with(entry: RecordEntry<Entry>) -> Record {
        ...
        // Struct literal rather than Record::new: on 0.6.2 that constructor takes
        // Option<Entry> and derives the RecordEntry variant itself, which cannot
        // produce Hidden or NA for a public Create action. The fields are public,
        // so all four variants stay reachable and the test can cover every way an
        // entry can be absent rather than only the convenient one.
        Record {
            signed_action: SignedHashed::with_presigned(
    ...
    fn present() -> Record {
        let entry = Entry::App(AppEntryBytes(
            SerializedBytes::try_from(attestation()).unwrap(),
        ));
    ...
    /// THE REGRESSION TEST. Before this fix it returned 2 pairs for 3 records and
    /// no error, so a caller that had already passed a quorum gate on 3 wrote an
    /// immutable HarmonyRecord claiming 2 participants.
    #[test]
    ...
    /// `Hidden` and `NA` are the other ways an entry can be absent. The point is
    /// that ANY absence aborts, not that one variant was special-cased.
    #[test]
