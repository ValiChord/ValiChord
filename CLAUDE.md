...
**⚠️ 0.7.0 IS NOW IN RELEASE-CANDIDATE (RC iterating; rechecked 2026-07-27):** `0.7.0-rc.0` tagged 2026-07-15, `0.7.0-rc.1` 2026-07-16, **rc.2, rc.3, and `0.7.0-rc.4` since** (rc.4 is the latest release as of 2026-07-27). This is the "watch for rc.0" trigger firing. Still NO plain `holochain-0.7.0` stable tag. **ETA revised down 2026-07-27 → early-to-mid August 2026** (was end-July/early-August on the 0.6-cycle rc.0→stable ≈ 2-week base rate). Three signals against an imminent stable tag, all verified on `develop`: two `!` breaking changes landed *after* rc.4 (#5898, #5906); the HDI validate-callback surface was reshaped twice in seven days (#5903, #5910 — see below); and the marquee 0.7 feature, `feat/5800-source-chain-restore-workflow`, is **still unmerged with no open PR** (9 ahead / 41 behind, last commit 2026-07-27). Treat this as a read of the churn rate, not a published schedule. Crate lines confirmed: `hdk 0.7.0-rc.x`, `hdi 0.8.0-rc.x`, `@holochain/client 0.9.0-rc.x`. Kitsune2 line confirmed from `Cargo.lock`: **rc.4 pins `kitsune2_* 0.5.0-dev.6`; our 0.6.2 pins `0.4.1`** — so anything landing on kitsune2's 0.5 line reaches us only at the 0.7 migration. Still HOLD on 0.6.2 until *stable*; then a deliberate planned migration, never auto.
...
- **`validate()` migration to `FlatOp v2` — BIGGER THAN "v1→v2", AND STILL MOVING (verified on `develop` 2026-07-27).** `flat_op_v2` module added to HDI; v2 `FlatOp` is built over new `dht_v2::Action`/`Op` types with validating constructors. All four ValiChord integrity zomes use `op.flattened::<EntryTypes, LinkTypes>()`, so all four are affected. **The FlatOp variants have since been reshaped twice, both after rc.0:**
  ...
  - **PR #5910** (merged 2026-07-27, +550/−121) — follow-up adding conversions *"found missing while porting a real app (dino-adventure) to 0.7.0-rc.3"*: `IntoEntryCreationData`, `Deref`, single-variant `TryFrom<Action>` narrowing with `try_from_action` siblings. **Partly reverts #5903**, restoring the `agent`/`new_key`/`original_key` fields on `CreateAgent`/`UpdateAgent`. Adds a doc note that narrowing failures should propagate as errors, not `ValidateCallbackResult::Invalid`.
  - **What this means for us:** our destructuring arms (e.g. `OpUpdate::Entry { app_entry, .. }`, anything reading a link's base/target/tag out of a variant) are exactly what moved to accessors. **Do not start this migration before the stable tag** — a real-app port was still shaking out missing conversions on rc.3, and the first refactor was partly walked back six days later. Re-read the HDI CHANGELOG at migration time rather than trusting this bullet.
- **`refactor!: re-layer conductor state types out of the holochain crate` (PR #5898, 2026-07-23)** — matters because `sweettest_integration` depends on the `holochain` crate directly. Expect import churn there at migration.
- **`feat(api)!: add paginated state dumps` (PR #5906, 2026-07-24)** — breaking admin-API change. Only matters if we read state dumps.
...
**Blocker-remover — ✅ LANDED on kitsune2 `main` 2026-07-27, but NOT YET REACHABLE FOR US.** What was tracked as branch `fix/491-stabilize-the-iroh-relay-hosted-in-bootstrap_srv` merged as **`3746be1` *"feat: stabilize authenticated iroh relay hosted in the bootstrap server"*** (refs #492; 16 files, ~+1160/−490). Relay access is gated by a bearer token on the relay WebSocket upgrade (`RelayConfig::with_auth_token`), validated in `AccessControl::on_connect`; bootstrap client gains `blocking_fetch_relay_token`; a client-side registration heartbeat (`relayReRegistrationIntervalS`, default 120 s) + token-rotating watchdog work around iroh 1.0.0 capturing the relay token once per connection actor; legacy `PUT /relay/register` allowlist retained for 0.4.x clients. Covered by unit, server-side auth-flow, and an end-to-end bootstrap-restart recovery test. **This should remove the "need a separate Iroh relay" blocker for both the deferred wind-tunnel kitsune live run and kangaroo desktop packaging — but not yet:** it landed *after* the newest kitsune2 release (`v0.5.0-dev.6`, 2026-06-30), so it is in no published version, and the 0.5 line is Holochain 0.7's, not our 0.6.2's (`0.4.1`). **Re-check at the 0.7 stable tag** whether the picked-up kitsune2 (`0.5.0-dev.7`+) actually carries `3746be1` before treating either blocker as cleared. **Note the holochain side has NOT consumed it:** `holochain/holochain`'s own `fix/491-…` branch is stale — last commit 2026-04-22, 114 behind `develop`, 1 commit ahead containing only `build: kitsune #491`.

**`holochain/holochain` branch watch — re-verified 2026-07-27 against `develop` (default branch is `develop`, not `main`; 57 branches).** Only three things are live; everything else previously listed here is stale:
- **LIVE** `feat/5800-source-chain-restore-workflow` — the marquee 0.7 feature, 9 ahead / 41 behind, last commit 2026-07-27 (*"add the chain restore workflow to conductor via an orchestrator"*), **no open PR**. Still does NOT recover private entries.
- **LIVE** `private-entry-sync-tests` → **PR #5912** (opened 2026-07-27) *"test: add DHT sync coverage for private entries"* — including *"gossip sync with private entries on either chain"* and *"op store never advertises an unservable private-entry op"*. **Test-only** (2 files: `test_utils/inline_zomes.rs`, `tests/gossip/mod.rs`), so nothing to act on — but private-entry gossip semantics are what DNA 1 and DNA 2 rest on. Watch in case the new coverage surfaces a real defect.
- **LIVE** `develop` itself.
