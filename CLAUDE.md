...
**🚨 0.7.0 STABLE IS BEING CUT RIGHT NOW — RELEASE PREPARED 2026-07-30, TAG NOT YET PUBLISHED.** Eric Harris-Braun announced on a live stream (2026-07-30, ~15:00 UK) that 0.7.0 is ready. Verified the same hour:

- **No `holochain-0.7.0` git tag yet** (`git/ref/tags/holochain-0.7.0` → 404), no GitHub release, crates.io `max_stable_version` still `0.6.3`. Latest published release is still `holochain-0.7.0-rc.5` (2026-07-29, `prerelease=true`).
- **BUT `develop` head is `d1ec5a72` *"chore: Prepare the 0.7.0 release"* (2026-07-30T14:16:58Z)** — 35 CHANGELOGs, +69/−35, and every one does exactly this:
  ```diff
  -default_semver_increment_mode: !pre_minor rc
  +semver_increment_mode: minor
  +default_semver_increment_mode: !pre_patch rc
  ```
  That is the switch telling the release automation the next cut is the **minor bump (0.7.0 stable)**, not rc.6 — and resetting the post-release default to `0.7.1-rc.x`. **The announcement is accurate; the tag is hours away, not weeks.** The old "early-to-mid August" ETA is superseded.

**Next tell is unchanged: the plain `holochain-0.7.0` tag. Still do NOT auto-upgrade when it lands — report to the user.**

⚠️ **MIGRATION IS BRANCH-ONLY — `main` STAYS ON 0.6.2.** User decision, 2026-07-30: the 0.7 migration is a major change and must happen on a dedicated **`v0.7.0` branch**, never directly on `main`. `main` remains on the working 0.6.2 stack until the branch is fully green (Tryorama + all 5 sweettest suites + UI e2e + a live demo round) and the user explicitly approves the merge. Rationale: what we have works and is publicly demoed; 0.7.0 being superior does not make a broken intermediate state acceptable. See `user_ceri_working_style` — core is paranoid.

---

**RC history (rc iterating; rechecked 2026-07-30):** `0.7.0-rc.0` tagged 2026-07-15, `0.7.0-rc.1` 2026-07-16, then rc.2, rc.3, rc.4, and `0.7.0-rc.5` (2026-07-29). This is the "watch for rc.0" trigger firing. Of the three signals recorded on 2026-07-27 as arguing against an imminent stable tag, **all three have now been overtaken by the prepare-release commit** — recorded below because the migration items in them are still real:
...
- ❌ **CORRECTED — "the marquee feature is the main brake" is WRONG. They are shipping 0.7.0 WITHOUT it.** `feat/5800-source-chain-restore-workflow` / **PR #5920** *"Add source chain restore workflow"* was **still `draft = true`, `mergeable = CONFLICTING`, review outstanding** at 2026-07-30T06:25Z — ~8 hours before the prepare-release commit landed. It is not in 0.7.0. **Consequence for our checklist: the source-chain-restore items below are almost certainly NOT in 0.7.0** — see the `AppStatus` bullet, which is now marked conditional. Verify against the stable CHANGELOG at migration rather than trusting either version of this note.

Treat the RC history as a read of the churn rate, not a published schedule. Crate lines: `hdk 0.7.0-rc.x`, `hdi 0.8.0-rc.x`, `@holochain/client 0.21.x` (see below). ❌ **CORRECTED 2026-07-30 — the earlier note "crates.io still shows `hdk 0.7.0-rc.4` / `hdi 0.8.0-rc.4` as newest, so rc.5's crate line is not published and an rc.5 trial migration could not resolve deps" is WRONG.** The rc.5 crates *are* published; the per-crate version numbers simply run **independently of the umbrella release name**. Verified on crates.io 2026-07-30: `holochain 0.7.0-rc.5` (17:49Z), `hdk 0.7.0-rc.4` (17:43Z), `hdi 0.8.0-rc.4` (17:41Z), `holochain_zome_types 0.7.0-rc.3` (17:33Z) — all published minutes *before* the `holochain-0.7.0-rc.5` tag at 17:57Z. **`hdk 0.7.0-rc.4` IS rc.5's hdk.** Never infer "not published" from a crate version that trails the release name. Kitsune2 line confirmed from `Cargo.lock`: **rc.5 pins `kitsune2_* 0.5.0` (stable — bumped from rc.4's `0.5.0-dev.6` by PR #5913); our 0.6.2 pins `0.4.1`** — so anything landing on kitsune2's 0.5 line reaches us only at the 0.7 migration. (Oddity, low significance: rc.5's lock leaves `kitsune2_dht` at `0.5.0-dev.6` while every sibling crate moved to `0.5.0` — looks like release sequencing.) Still HOLD on 0.6.2 until *stable*; then a deliberate planned migration, never auto.
...
- ⚠️ **CONDITIONAL — PROBABLY NOT IN 0.7.0. VERIFY FIRST, DO NOT PLAN WORK ON IT.** The source-chain-restore PR (#5920) was still a conflicting draft when the 0.7.0 release was prepared (see the corrected bullet above), so everything in this item is likely absent from 0.7.0 and lands in a later release instead. **Check the stable CHANGELOG before touching `dev-setup.mjs` or the Svelte `AppStatus` handling.** If/when it does ship: new `AppStatus` variants `AwaitingRestore` (restore in progress) and `Unrecoverable(cell_id, reason)` (terminal — chain forked or warrant validated) — `dev-setup.mjs` and Svelte UI currently assume only `Running`/`Disabled`; new `SystemSignal` variants `RestoreComplete { cell_id }`, `AppRestoreComplete { installed_app_id }`, `RestoreFailed { cell_id, reason }`; new conductor config field `restore_chain_quorum: u8` (default 2). (Source: `holochain/holochain` branch `cascade-read-and-cutover`, `docs/design/source_chain_restore.md`)
