...
The fastest way to see the protocol. Click **Run Protocol** and watch the live Holochain network on Oracle Cloud run a full commit-reveal validation in ~2 minutes — researcher and three validators committing blind and revealing simultaneously. Real network time, not a timer.

At the end of each run the page shows a `curl` command so you can fetch the HarmonyRecord directly from the Oracle DHT and confirm the result was not generated by this website.

🎬 **[Watch a video of the current demo running end-to-end →](https://www.linkedin.com/posts/ceri-john-2300a855_reproducibility-openscience-researchintegrity-activity-7466596558680018944-zlgn)**

**[Full technical guide — Flask architecture, request flow, Render deployment →](https://github.com/topeuph-ai/ValiChord/blob/main/demo/DEMO_WEBSITE.md)**
...
**v0.5.4 (May 2026):** Security hardening sweep — warrant gates added to four coordinator entry points (`submit_validation_request`, `publish_validator_profile`, `assess_difficulty`, `link_agent_identity`); integrity validation added for `ResearcherResultCommitment` (32-byte hash) and `HarmonyRecord` (`validator_types` parallel-array length); TypeScript `BadgeType` serde strings corrected (`"GoldReproducible"` etc.); 6 new sweettest tests (attestation tests 16–20, governance test 17) bringing total coverage to 20 + 17 tests.
