...
Routine version check surfaced that **0.7.0 is now in RC**: `0.7.0-rc.0` (2026-07-15) and `0.7.0-rc.1` (2026-07-16). This is the "watch for rc.0" trigger from the 2026-06-13 estimate firing. **Rechecked 2026-07-27: rc.2, rc.3 and `0.7.0-rc.4` have since shipped (rc.4 is the latest release); still no plain `holochain-0.7.0` stable tag.** **ETA revised 2026-07-27 → early-to-mid August 2026** (was end-July/early-August): two `!` breaking changes landed on `develop` *after* rc.4 (#5898 re-layers conductor state types out of the `holochain` crate — which `sweettest_integration` depends on directly; #5906 paginated state dumps), the HDI validate-callback surface was reshaped twice in seven days (#5903, #5910), and the marquee 0.7 feature `feat/5800-source-chain-restore-workflow` is **still unmerged with no open PR**. Crate lines: `hdk 0.7.0-rc.3`, `hdi 0.8.0-rc.3`, `@holochain/client 0.21.0-rc.1` (**not** 0.9.x — that is the Rust `holochain_client` crate; the two were previously conflated). **We stay on 0.6.2 until 0.7.0 is *stable*; the migration is deliberate and planned, never auto.**
...
**Blocker-remover — ✅ LANDED 2026-07-27, not yet reachable for us.** What was tracked as branch `fix/491-stabilize-the-iroh-relay-hosted-in-bootstrap_srv` merged to kitsune2 `main` as **`3746be1` — *"feat: stabilize authenticated iroh relay hosted in the bootstrap server"*** (refs #492; 16 files, ~+1160/−490). It puts an authenticated iroh relay *inside* `kitsune2-bootstrap-srv`: relay access gated by a bearer token on the relay WebSocket upgrade (`RelayConfig::with_auth_token`), validated in `AccessControl::on_connect`; bootstrap client gains `blocking_fetch_relay_token`; a client-side registration heartbeat (`relayReRegistrationIntervalS`, default 120 s) plus a token-rotating watchdog work around iroh 1.0.0 capturing the relay token once per connection actor; the legacy `PUT /relay/register` allowlist stays for 0.4.x clients. Unit + server-side auth-flow + end-to-end bootstrap-restart recovery tests. **This should clear the "separate Iroh relay" prerequisite for both the deferred wind-tunnel kitsune live run and kangaroo desktop packaging — but not yet:** it landed *after* the newest kitsune2 release (`v0.5.0-dev.6`, 2026-06-30), so it is in no published version, and the 0.5 line belongs to Holochain 0.7, not our 0.6.2 (`Cargo.lock` verified: `0.7.0-rc.4` → `kitsune2_* 0.5.0-dev.6`; `0.6.2` → `0.4.1`). **Re-check at the 0.7 stable tag** that the picked-up kitsune2 actually carries `3746be1` before treating either blocker as cleared. Note the **holochain side has not consumed it** — that repo's own `fix/491-…` branch is stale (last commit 2026-04-22, 114 behind `develop`).

**Branch watch re-verified 2026-07-27** across all 57 `holochain/holochain` branches (default branch is `develop`). Only three things are live: `develop` itself; `feat/5800-source-chain-restore-workflow` (marquee 0.7 feature, last commit 2026-07-27, still unmerged with no PR, and still does not recover private entries → `ValidatorPrivateAttestation`/`LockedResult` lost on restore); and PR **#5912** *"test: add DHT sync coverage for private entries"* (test-only, but DNA 1/DNA 2 territory — watch in case it surfaces a real defect). ⚠️ **`feat/generate-ts-types-ts-rs` is dead, not active** — last commit 2025-12-11, message `wip`, 257 behind; the hand-maintained `valichord-ui/types.ts` mirror has no replacement coming. Nine other previously-watched branches confirmed stale.
