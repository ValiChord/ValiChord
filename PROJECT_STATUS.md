...
Routine version check surfaced that **0.7.0 is now in RC**: `0.7.0-rc.0` (2026-07-15) and `0.7.0-rc.1` (2026-07-16). This is the "watch for rc.0" trigger from the 2026-06-13 estimate firing. **Rechecked 2026-07-30: rc.2, rc.3, rc.4 and `0.7.0-rc.5` (2026-07-29) have since shipped; still no plain `holochain-0.7.0` stable tag.** **ETA unchanged at early-to-mid August 2026 — but the reasoning behind it has been corrected and softened.** Of the three signals recorded on 2026-07-27:

- **One was a misread and is now corrected.** #5898 (re-layers conductor state types out of the `holochain` crate — which `sweettest_integration` depends on directly) and #5906 (paginated state dumps) were recorded as landing *after* rc.4. They are **in rc.4** — verified by comparing each merge commit against the rc.4 tag; their 07-23/07-24 merge dates precede rc.4's 07-27 tag. Both remain real migration items; neither is evidence of post-rc churn.
- **The HDI churn signal has weakened.** rc.5 absorbed **#5910**, so the validate-callback surface our four integrity zomes depend on is now inside a release rather than moving on `develop` — the first time in three weeks. rc.4→rc.5 introduced **no new `!` breaking commits** (9 commits / 80 files).
- **The marquee feature now has a PR, and that PR is the remaining brake.** `feat/5800-source-chain-restore-workflow` is **PR #5920** (opened 2026-07-29, +2516/−33 across 15 files, last commit 2026-07-30) — but it is a **draft with merge conflicts** and review outstanding. Closer than "no open PR"; not days away.

**A published kitsune2 stable now carries the relay work — see the blocker-remover note below.** Crate lines: `@holochain/client 0.21.0-rc.1` (**not** 0.9.x — that is the Rust `holochain_client` crate; the two were previously conflated); crates.io still shows `hdk 0.7.0-rc.4` / `hdi 0.8.0-rc.4` as newest, so **rc.5's crate line is not on crates.io yet** and an rc.5 trial migration could not resolve deps today. **We stay on 0.6.2 until 0.7.0 is *stable*; the migration is deliberate and planned, never auto.**
