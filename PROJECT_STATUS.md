...
- Verified conductor-free (`--check`): success, noop, **fail-closed** refusal on tampered sha256, pin ceiling holds/allows. **Only remaining step is ops, not code:** the first live Oracle rollout (publish a no-op revision matching the currently-applied coordinators, flip `AUTOUPDATE=on` on the 4 containers, verify markers, then publish a real bump).
