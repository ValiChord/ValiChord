...
**Validation.** Each validator independently downloads the data, sets up the computational environment, runs the code, and records whether they get the same results. They document their process, time invested, barriers encountered, and confidence level. Results are submitted through a blind commitment protocol — each validator privately seals their findings and a cryptographic hash of their assessment before anyone else's are visible. Once every validator has committed, the reveal window opens and validators submit their public attestations. The commitment hash (SHA-256 of the assessment content concatenated with a private nonce) means a validator cannot change their assessment after committing it — the hash binds their sealed finding to their public reveal.
...
The blind commitment protocol is central: each validator privately seals their findings before seeing anyone else's, and a cryptographic hash of their sealed assessment is published to the shared DHT as a `CommitmentAnchor`. This prevents the last validator from adjusting their results to match the majority — a well-known attack vector in any system where independent assessors can see each other's results before submitting. The hash binds the reveal to the pre-declared content: `commitment_hash = SHA-256(assessment_bytes || nonce)`. At reveal time, any participant can verify that what a validator revealed matches what they committed to. Separately, the researcher publishes a `ResearcherResultCommitment` to the shared DHT before validation begins — a hash of their own claimed result — so validators can verify the researcher did not adjust their result after seeing validator findings. This is the standard cryptographic pattern known as commit-reveal, implemented using Holochain's native private entries and SHA-256 hash commitments. (True Holochain countersigning — mathematically enforced simultaneous atomic reveal — is a Phase 2 upgrade; the current hash-commitment approach provides equivalent anti-manipulation guarantees for Phase 0/1 at lower operational complexity.)
