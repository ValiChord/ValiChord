...
<div align="center">
  <img src="https://github.com/ValiChord/ValiChord/blob/main/Images/Valichord%20logo-standard%20v2-1.5x.jpeg" width="550px" alt="ValiChord Logo">
</div>
...
- **Privacy by architecture.** Sensitive data stays local with the researcher or institution and never enters the shared DHT. Only one-way cryptographic hashes travel to the network. The hash proves the data existed and hasn't been tampered with, but cannot be reversed to recover the data itself. This is data minimisation enforced structurally — not merely a policy commitment. Where erasure rights do apply under GDPR, they can be exercised against the researcher's private data without touching the shared attestation record. For non-sensitive computational research, an *open audit mode* (see Layer 0) complements this: the dataset is posted to the shared DHT in encrypted form at submission; after reveal, the researcher publishes the decryption key as part of the Harmony Record, allowing any third party — not just the two parties to the validation — to independently verify the data against the original commitment hash. The blind commit-reveal, phase transitions, and Harmony Record are identical in both modes; only the data locality changes.
...
ValiChord solves this using content-addressed verification: when data is submitted, each participant's node generates a cryptographic fingerprint of every file. Research files — data, code, protocol documents — are fingerprinted using SHA-256, the standard used by academic repositories and broadly supported by verification tools. Holochain's own internal addressing uses BLAKE2b for its attestation records. These are separate but complementary layers: the SHA-256 fingerprint identifies the research materials; Holochain's addressing identifies the validation actions performed on them. Change a single bit of the original data and the fingerprint changes completely — so any tampering is immediately detectable. Anyone can verify, at any time, that their copy matches every other copy. Where the data lives during a validation round matters as much as the fingerprint itself. Uploading to a public repository such as Zenodo at submission time would make the research — including potentially the methodology and intermediate results — publicly readable before any validator has committed. This would undermine the blind protocol: a validator who can read the researcher's outputs before sealing their own assessment is not independent. The correct model is encrypted or access-controlled storage during the validation period, with public archival to Zenodo, Figshare, or an institutional repository triggered only after the reveal — when all parties have published simultaneously and the Harmony Record is written. At that point, Zenodo is exactly the right destination: permanent, citable, and redundantly hosted. A study validated in 2027 can still be checked in 2035. The fingerprint committed at submission guarantees the archived data is identical to what was validated, regardless of when it becomes publicly readable.

**Two data locality modes.** By default — and always for GDPR-sensitive research — the dataset stays in the researcher's private DNA and never enters the shared DHT. The commitment hash travels to the network; the data does not. Third parties can audit the entire process (the hash chain, phase transitions, attestations, and Harmony Record are all publicly readable on the DHT) but cannot independently re-verify the underlying data without access to the researcher's system. For non-sensitive computational research, *open audit mode* changes only the data locality: the dataset is encrypted with Holochain's native X25519 key and posted to the shared DHT at submission alongside the commitment hash. After all validators reveal, the researcher publishes the decryption key in the Harmony Record. Any party — journal editor, funder, independent scientist, or automated checking system — can then decrypt the data, hash it, and compare against the original commitment, with no need to access the researcher's private system or trust ValiChord's word. This closes the last remaining gap in full third-party verifiability. Patient data and any GDPR-regulated datasets remain in GDPR mode regardless of domain; the mode is set at submission and cannot be changed after the commitment is posted. Open audit mode is architecture-complete; the submission UI and key management workflow are Phase 1 work.
...
**A proposed extension: the Two-Round Protocol.** The current protocol has a subtle timing gap worth noting: validators decide *Reproduced / Not Reproduced* before formally seeing the researcher's committed values — they base that judgment on the paper's published claims. The researcher's formally committed values arrive on the network at the exact same moment the validators reveal their findings, so the comparison happens in the background rather than as a deliberate step. For most straightforward studies this makes no difference. But for complex studies — contested tolerances, partial matches, ambiguous methodology — there is genuine value in making the formal verdict *after* the researcher's committed values are on the table.

The two-round protocol addresses this by splitting the process into two separate sealed steps. In Round 1, validators run the study and seal only their raw findings — what their execution produced — with no verdict yet. Once all raw findings are revealed, the researcher's committed values are formally published and visible to everyone. In Round 2, each validator independently and privately seals their verdict — *Reproduced / Partially Reproduced / Failed to Reproduce* — based on formally comparing their findings to the researcher's committed values, still without seeing what any other validator concluded. Only after all verdicts are revealed does the Harmony Record get produced. The same blind structure applies throughout: no validator can see another's verdict before sealing their own. The verdict itself uses exactly the same outcomes as today — nothing changes for journals, funders, or badge systems. The only difference is that the formal comparison is a structured, documented step in the record rather than a background inference. This feature is opt-in at the researcher's choice, applies to human validators only, and is proposed but not yet implemented — the full specification is in *doc 23 — Two-Round Protocol Proposal*.
...
This serves two functions: accountability (any decision can be audited) and trust (external parties can independently verify the entire validation history of any study without trusting ValiChord itself). In GDPR mode, this guarantee covers the process: the hash commitments, phase transitions, and attestations are all on the DHT and independently auditable by anyone. In open audit mode, it extends to the data itself: the post-reveal decryption key allows any third party to verify that the underlying dataset matched the original commitment hash — the Harmony Record becomes fully self-verifying without relying on any central authority.
...
**Automated CI/CD approaches** (Docker, containerised pipelines, GitHub Actions) can verify that code *runs* and produces *outputs*. They cannot assess whether those outputs *make sense*. Automated testing cannot flag physically impossible intermediate values, notice that a data preprocessing step is undocumented, or identify that code ran but produced garbage because of an environment difference. The gap between "code executed without errors" and "results are scientifically reproducible" is real — but it does not follow that the only thing that can cross it is a human.

ValiChord's protocol is neutral about whether a validator is a human researcher or an AI agent. What the protocol requires is independence, a sealed commitment before the reveal, and a judgment against pre-registered thresholds. The Feynman integration — an AI research agent that submits replication findings directly to ValiChord and receives a cryptographically sealed Harmony Record — is the first demonstration of this in practice.

The division of labour that is emerging is this: humans provide what automation genuinely cannot — physical execution in a wet lab, domain expertise in interpreting ambiguous protocol conditions, and the embodied judgment required when an experiment involves equipment, materials, or environmental variables that no model has seen. AI validators apply the judgment layer: checking whether a result meets a pre-declared threshold consistently, without the social desirability bias that leads human validators to call "close enough" as Reproduced out of encouragement rather than evidence. Multiple AI validators, each independently sealing their verdict in the commit-reveal protocol, provide the same independence guarantees as a human panel — with the added property that their judgments are not subject to the quiet pressures of professional courtesy.

Automated tools — including AI validators — handle the bulk of computational validation. That economic efficiency is precisely what makes the human execution layer viable: labs with the physical capacity and domain expertise to reproduce experimental work can be paid for that execution, without the cost being driven up by requiring human judgment at every step.
