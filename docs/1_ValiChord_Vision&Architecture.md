...
**Validation.** Each validator independently downloads the data, sets up the computational environment, runs the code, and records whether they get the same results. They document their process, time invested, barriers encountered, and confidence level. Results are submitted through a blind commitment protocol — each validator privately seals their findings and a cryptographic hash of their assessment before anyone else's are visible. Once every validator has committed, the reveal window opens and validators submit their public attestations. The commitment hash (SHA-256 of the assessment content concatenated with a private nonce) means a validator cannot change their assessment after committing it — the hash binds their sealed finding to their public reveal. The researcher published their own result commitment (a hash of their structured metrics) at submission time; at reveal, they publish the verified reveal alongside the validators, making both sides' ground truth available on the same shared DHT for comparison.
...
The blind commitment protocol is central and now fully symmetric — both validators and the researcher commit before validation begins, and both reveal simultaneously when all validators have committed. The hash binds each party's reveal to their pre-declared content: `commitment_hash = SHA-256(msgpack(content) || nonce)`. At reveal time, any participant can verify that what was revealed matches what was committed. The validator side: each validator privately seals their `ValidationAttestation` in their DNA 2 workspace; `seal_private_attestation` generates a nonce and computes the hash; DNA 2's `post_commit` forwards the hash to DNA 3's `CommitmentAnchor` on the shared DHT. The researcher side: at study submission, the researcher calls `lock_researcher_result` in their private DNA 1, which generates a nonce, computes `SHA-256(rmp_serde::to_vec_named(metrics) || nonce)`, stores the structured metrics and nonce privately, and automatically publishes only the hash to DNA 3 via `publish_researcher_commitment` — months before any validator begins work. At reveal, the researcher retrieves their private nonce from DNA 1 and calls `reveal_researcher_result` in DNA 3, which verifies the hash on-chain and publishes the verified structured metrics as a `ResearcherReveal` entry. Once all reveals are complete, DNA 4 assembles the `HarmonyRecord` with the researcher's pre-committed expected values and each validator's independently reproduced values both on the public DHT — the basis for a genuine, cryptographically verifiable comparison. (True Holochain countersigning — mathematically enforced simultaneous atomic reveal — is a Phase 2 upgrade; the current hash-commitment approach provides equivalent anti-manipulation guarantees for Phase 0/1 at lower operational complexity.)
...
**Built and integration-tested.** The four-DNA Holochain infrastructure described in this document has been fully implemented in Rust and tested. As of March 2026, **87 integration tests pass** against live Holochain conductors via the Tryorama framework. Tests launch up to 7 independent conductor processes — each with its own agent identity, source chain, and DHT participation — executing the full blind commit-reveal protocol and producing a Harmony Record on a shared live DHT. The Technical Reference documents the architectural intent; the authoritative implementation is under `valichord/dnas/`. The transition from design document to tested infrastructure followed Arthur Brock's February 2026 solution engineering review, which identified that the original scaffold needed reorganisation around Holochain's four-DNA membrane pattern rather than the eight-layer conceptual framework. That reorganisation is now complete and tested. The cryptographic commit-reveal protocol is now fully symmetric (March 2026): validators commit via `seal_private_attestation` (DNA 2) → `CommitmentAnchor` on DNA 3; researchers commit via `lock_researcher_result` (DNA 1) → `ResearcherResultCommitment` on DNA 3. At reveal, validators call `submit_attestation` and the researcher calls `reveal_researcher_result` (DNA 3), which verifies the hash on-chain before writing `ResearcherReveal` to the DHT. Both parties' structured metrics are then publicly available for comparison — closing the loop from commitment-as-proof-of-action to a fully verifiable, cryptographically-bound comparison of researcher-declared and independently-reproduced results.
