...
The right-of-reply mechanism must be low-friction by design. This is not a UX nicety — it is a governance requirement. The right of reply is the legitimate alternative to retraction. If submitting a response is cumbersome, time-consuming, or technically intimidating, frustrated researchers will not use it. They will instead lobby for retraction, appeal through institutional channels, or simply apply political pressure — exactly the pressure the governance framework is designed to resist. A pathway that exists in principle but is difficult in practice is not a pathway.

The implementation implication: a researcher should be able to submit a right-of-reply response through a single form — plain text, no specialist tooling, no Holochain knowledge required — and have it cryptographically attached to the Harmony Record hash automatically. The response must appear in the same view as the HarmonyRecord, not behind an additional click. If the attachment process requires manual governance intervention, that latency must have a defined maximum (proposed: 5 business days from submission to attachment). Unanswered replies — responses submitted but not yet attached — must be surfaced as pending in the record view, so that the absence of a response is not mistaken for acceptance of the finding.

*Resolution phase: The `ResearcherReply` entry type and coordinator function are Phase 1 implementation tasks. The UI/UX flow (single-form submission, automatic attachment, pending-state display) must be specified before Phase 1 deployment.*
...
The HTTP gateway and any UI rendering Harmony Records must surface supersession explicitly and prominently — not behind a click, not in metadata. The display requirement is: original record shown with a visible banner ("Superseded — reason: [type] — see [correction record hash]"), the correction record linked directly, and the GovernanceDecision authorising the correction one further click away. This is an API licence requirement parallel to the disagreement salience rule in the Anti-Domestication Mechanics.

**GDPR / court-ordered full erasure.** The hashed-institution approach described above provides functional unlinkability (destroying the key renders the hash meaningless) without touching the DHT. This satisfies the spirit of GDPR data minimisation under Article 25. However, if a court order in a specific jurisdiction mandates full erasure — not just unlinkability but actual deletion of bytes from storage — ValiChord has no technical mechanism to comply. Holochain's DHT provides no coordinated deletion primitive. In this scenario, the honest answer is that ValiChord cannot guarantee compliance with such an order beyond good-faith efforts to contact node operators in the relevant jurisdiction and request local deletion. This is a material legal risk that must be disclosed to validators before they join the network, and it argues strongly for the hashed-institution architecture being implemented before Phase 1 rather than deferred.
