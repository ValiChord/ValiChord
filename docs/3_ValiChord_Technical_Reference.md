...
- **Phase 1 planned: native multi-device agent linking.** ValiChord will implement its own `AgentIdentityAttestation` entry type directly in the attestation DNA — no external service dependency. The protocol: both devices sign a canonical 78-byte payload (the two raw `AgentPubKey` bytes in lexicographic order); one device calls `link_agent_identity({ other_agent, other_signature })`; the coordinator verifies the external signature and signs its own half, committing the entry with `AgentToIdentityAttestation` links from both pubkeys for symmetric lookup. Revocation is via `delete_entry`, gated to one of the two named agents. A `PendingIdentityLink` type supports the async ceremony (devices not online simultaneously). **Requires an attestation integrity zome change (new DNA hash) — schedule as a deliberate network reset.** Once live, `person_key` in `ValidatorProfile` and `ValidatorReputation` is populated from `get_linked_agents` and used as the stable primary key for profile lookup, reputation aggregation, and COI checks. Future integration note: Flowsta's `agent_linking` zome implements the identical protocol and could replace ValiChord's native version if a shared cross-application identity graph becomes useful later.
