...
- **Production reputation gap surfaced (April 2026):** `_update_reputation_internal` is gated behind `system_coordinator_key.is_empty()` so it is a no-op in production — the write would fail `validate()`. `get_validator_reputation` therefore returns nothing in production and every validator's effective `CertificationTier` is `Provisional`. Consequence: the badge thresholds in `evaluate_badge` (Gold ≥7 `ExactMatch`, Silver ≥5, Bronze ≥3) count credentialed DHT participants, not validators with any verified track record. A Gold badge can be issued by seven first-time validators. This is the intended Phase 0/Phase 1 boundary — Phase 1 wires in a trusted `update_validator_reputation` caller (oracle or on-chain coordinator key). Until then, badge tier reflects participant count and agreement level only, not validator experience. All public-facing descriptions of badge tiers must reflect this. See Engineer Handover Known Architectural Gaps for remediation plan.
