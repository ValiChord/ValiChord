...
**v0.5.x additions — `valichord_attestation/` (now format v2, 584 tests as of 2026-08-22):** A Python library that extends ValiChord's commit-hash-reveal principle to AI evaluation runs. A bundle is a canonical, deterministically hashed JSON document (RFC 8785 / JCS) containing reported metrics and a SHA-256 Merkle root over per-sample outputs. Any individual sample can be proven to a verifier without disclosing the full log. Two hashes: `bundle_hash` (full identity, includes `meta`) and `content_hash` (scientific equivalence, excludes `meta`). `Metric.filter` disambiguates metrics sharing the same key from different scorer passes. Probabilistic challenge-response (v0.5.0): verifier-supplied nonce → HMAC-SHA256 seed → SHA-256 counter-mode index PRNG → k challenged samples proven via Merkle paths. `InspectEvalsAdapter` reads `evaluation_report:` blocks from `eval.yaml`; optional `eval_yaml_metadata=` folds task-level provenance (arxiv, group, human baseline, floating-asset reproducibility warnings) into `Bundle.meta`. `InspectAILogAdapter` reads inspect_ai `.eval`/`.json` log files directly via the inspect_ai Python API. Format v2 (2026-08-18) changed the Merkle construction to RFC 6962 §2.1 — leaf/node domain separation, odd subtrees promoted rather than duplicated — and nothing else; v1.x bundles remain valid and are verified under the earlier construction. The Holochain DHT integration (bundles as on-chain attestations) is still ahead and is **not** what the format version refers to. See `valichord_attestation/spec/attestation_format_v2.md` (and `_v1.md`, still normative for v1.x) and `docs/13_Valichord_Engineer_Handover.md` § `valichord_attestation`.
