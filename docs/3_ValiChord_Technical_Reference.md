...
- **4-tier certification system + tier-weighted badge thresholds (April 2026):** `CertificationTier` now has four levels: `Provisional` (default) → `Standard` (≥5 completed rounds) → `Advanced` (≥20 rounds + agreement rate ≥60%) → `Certified` (≥50 rounds + rate ≥80%). Thresholds are placeholder values to be calibrated from real data. `cert_tier()` in `governance_coordinator` computes the tier after each `update_validator_reputation` call. Badge thresholds in `evaluate_badge` now use tier-filtered counts (`QualifiedCounts`): Gold requires **7+ Advanced/Certified** validators; Silver requires **5+ Standard/Advanced/Certified** validators; Bronze and FailedReproduction count all validators regardless of tier (`any >= 3`). Implication in production: since all validators are `Provisional` until the Phase 1 oracle is live, `standard_plus` and `advanced_plus` are always 0 in production — Gold and Silver cannot be issued in production until reputation is populated. Bronze and FailedReproduction remain fully functional. Sweettest tests 12 and 13 verify promotion: 5 `Reproduced` rounds → `Standard`; 4 rounds → stays `Provisional`. Note: the integrity-layer `evaluate_badge_type()` still validates against raw participant count (cross-DNA DHT lookups are unavailable in `validate()`) — the coordinator enforces the stricter tier-filtered rule; this divergence is an acknowledged Phase 1 limitation.
