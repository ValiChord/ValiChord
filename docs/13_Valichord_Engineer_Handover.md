...
- **Multi-device identity / agent linking** — `ValidatorReputation` (DNA 4) and `ValidatorProfile` (DNA 3) are both keyed by `AgentPubKey`, which is a device key, not a person-stable identity. If a validator replaces their device, they receive a new `AgentPubKey` and their reputation history is stranded on the old key. Their new key starts with zero reputation and no profile. For Phase 0 (institutional validators on institutional hardware) this is acceptable. For Phase 1 it is not — reputation scores become real operational signals and key continuity matters. **Partial fix landed March 2026:** both `ValidatorReputation` and `ValidatorProfile` now carry `person_key: Option<AgentPubKey>` (`#[serde(default)]`, fully backwards-compatible). When a cross-device identity system populates this field, reputation aggregation and profile lookup should use it as the stable primary key instead of the device `AgentPubKey`. The `UpdateValidatorProfileInput` struct also carries `person_key: Option<Option<AgentPubKey>>` so validators can set it via `update_validator_profile`. **What remains for Phase 1:** (1) population logic — resolve the canonical person key via Flowsta `IsSamePersonEntry` or Deepkey and write it into the field; (2) aggregation — `_update_reputation_internal` currently always passes `person_key: None`; update to pass the resolved person key when available; (3) `get_validator_profile` and COI checks to compare against the full linked-key set. The ecosystem solution is the Flowsta `agent_linking` zome (`github.com/WeAreFlowsta/flowsta-identity-dna`). Coordinate with Holochain 1.0 Deepkey (key rotation, see Upgrade Radar below) — the two mechanisms are complementary, not competing.
