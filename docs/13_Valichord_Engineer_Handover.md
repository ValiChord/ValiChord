...
| `ReproducibilityBadge` type not integrity-enforced | Claude red-team audit (2026-04-20) | High — protocol integrity | `governance_integrity` now contains an `evaluate_badge_type()` helper function that validates badge type against raw participant count (Gold ≥7 ExactMatch, Silver ≥5, Bronze ≥3, FailedReproduction ≥3 Divergent/UnableToAssess). The `ReproducibilityBadge` create validate arm calls it and rejects via `std::mem::discriminant` comparison if `badge.badge_type` does not match what the agreement+count would produce. **Coordinator divergence (2026-04-24):** `evaluate_badge()` in `governance_coordinator` was subsequently updated to use tier-filtered `QualifiedCounts` (Gold requires 7+ Advanced/Certified validators, Silver requires 5+ Standard+ validators) rather than raw count. `evaluate_badge_type()` in integrity still uses raw count — cross-DNA reputation lookups are unavailable in `validate()`. The coordinator is therefore more conservative than the integrity layer for Gold/Silver; the tier-filtered rule is coordinator-only enforcement. This is an acknowledged Phase 1 limitation consistent with the existing note that content correctness is enforced at the coordinator layer, not at the network validation layer. |
...
- **Production reputation is static — Gold and Silver badges cannot be issued in production (Phase 1).** `_update_reputation_internal` is gated behind `system_coordinator_key.is_empty()` (the dev/test bypass) so it never runs in production — the write would fail `validate()` which requires the coordinator key to be the author. All validators therefore remain at `CertificationTier::Provisional` in production until the Phase 1 oracle is live. **Tier system (2026-04-24):** four tiers are now defined — `Provisional` (default) → `Standard` (≥5 rounds) → `Advanced` (≥20 rounds + agreement rate ≥60%) → `Certified` (≥50 rounds + rate ≥80%). Badge thresholds now use tier-filtered `QualifiedCounts`: Gold requires 7+ `Advanced`/`Certified` validators; Silver requires 5+ `Standard`/`Advanced`/`Certified` validators; Bronze and FailedReproduction count all validators regardless of tier. **Production implication:** with all validators `Provisional`, `standard_plus` and `advanced_plus` are always 0 — Gold and Silver cannot be issued in production. Bronze (any ≥3) and FailedReproduction (any ≥3) remain fully functional. This is more conservative than the previous raw-count system (where Gold could be issued by seven first-time validators). Phase 1 remediation: a trusted off-chain oracle (or designated `system_coordinator_key` holder) calls `update_validator_reputation` after each round; once validators reach `Standard` or above, Silver and Gold become achievable. **Dev/test mode:** promotion runs automatically — sweettest tests 12 and 13 verify Provisional→Standard at 5 rounds and the boundary at 4.
