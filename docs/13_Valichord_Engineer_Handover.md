...
- **Multi-device identity / agent linking** — `ValidatorReputation` (DNA 4) and `ValidatorProfile` (DNA 3) are both keyed by `AgentPubKey`, which is a device key, not a person-stable identity. If a validator replaces their device, they receive a new `AgentPubKey` and their reputation history is stranded on the old key. For Phase 0 (institutional validators on institutional hardware) this is acceptable. For Phase 1 it is not — reputation scores become real operational signals and key continuity matters. **Partial fix landed March 2026:** both `ValidatorReputation` and `ValidatorProfile` carry `person_key: Option<AgentPubKey>` (`#[serde(default)]`). When populated, this is the stable primary key; `UpdateValidatorProfileInput` carries the corresponding field so validators can set it via `update_validator_profile`. **Phase 1 planned implementation (native, no external dependency):** Add `AgentIdentityAttestation { agent_a: AgentPubKey, signature_a: Signature, agent_b: AgentPubKey, signature_b: Signature }` entry type and `AgentToIdentityAttestation` link type to `attestation_integrity`. **Note: this changes the attestation DNA hash — schedule as a deliberate network reset, not an incidental patch.** Coordinator functions to add: `link_agent_identity({ other_agent, other_signature })` — verifies the external signature via `verify_signature`, signs the same canonical payload (two 39-byte raw pubkeys concatenated in lexicographic order) with the caller's key via `sign()`, commits the entry and writes `AgentToIdentityAttestation` links from both pubkeys for symmetric lookup; `get_linked_agents(agent) → Vec<AgentPubKey>`; `revoke_agent_identity_link(attestation_hash)` — gated to one of the two named agents. Add a `PendingIdentityLink` link type for the async ceremony (device A offline when B initiates). After this is live: (1) `update_validator_profile` can auto-populate `person_key` from `get_linked_agents` on first call from a new device; (2) `_update_reputation_internal` passes the resolved `person_key`; (3) COI checks union all linked keys. **Future integration note:** Flowsta's `agent_linking` zome (`github.com/WeAreFlowsta/flowsta-identity-dna`) implements this identical protocol and could replace ValiChord's native implementation if a shared cross-application identity graph becomes useful. The data model is compatible — `IsSamePersonEntry` uses the same two-signature sorted-payload scheme. Coordinate with Holochain 1.0 Deepkey (key rotation) — the two mechanisms are complementary.
