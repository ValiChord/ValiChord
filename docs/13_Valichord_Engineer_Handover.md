...
| `ReproducibilityBadge` type not integrity-enforced | Claude red-team audit (2026-04-20) | High — protocol integrity | `governance_integrity` now contains an `evaluate_badge_type()` helper function that mirrors `evaluate_badge()` in the coordinator (same thresholds: Gold ≥7 ExactMatch, Silver ≥5, Bronze ≥3, FailedReproduction ≥3 Divergent/UnableToAssess). The `ReproducibilityBadge` create validate arm calls it and rejects via `std::mem::discriminant` comparison if `badge.badge_type` does not match what the agreement+count would produce. Previously the coordinator's badge-issuance logic was the sole enforcement point — a coordinator bypass could issue a Gold badge for a Divergent round. |
...
- **Production reputation is static — badge counts are participant counts, not certified-validator counts (Phase 1).** `_update_reputation_internal` is gated behind `system_coordinator_key.is_empty()` (the dev/test bypass) so it never runs in production — the write would fail `validate()` which requires the coordinator key to be the author. `get_validator_reputation` therefore returns no record in production, and every validator's effective `CertificationTier` is `Provisional`. Downstream consequence: the badge thresholds in `evaluate_badge` / `evaluate_badge_type` (Gold ≥7 `ExactMatch`, Silver ≥5, Bronze ≥3) count credentialed DHT participants, not validators with any verified track record. **A study can receive a Gold badge from seven first-time validators.** This is an architectural gap, not a bug — it is the intended Phase 0/Phase 1 boundary. Phase 1 remediation: a trusted off-chain oracle (or a designated on-chain `system_coordinator_key` holder) calls `update_validator_reputation` explicitly after each round. Until that is wired up, all communications about badge tier should note that tier reflects participant count and agreement level, not validator experience. Do not add reputation-weighted badge thresholds before the reputation population mechanism is live — the thresholds would be evaluated against empty/`Provisional` records and behave identically to the current count-only logic.
