...
| Reveal-phase quorum gate counted links, not distinct validators | Independent review (2026-06-10), verified by Claude Opus 4.8 | Medium — liveness/griefing | `check_all_commitments_sealed_inner` (DNA 3) decided quorum with `commitment_links.len() >= required`. Because `attestation_integrity` does not constrain `RequestToCommitment` link *creation* (it only makes existing links immutable — there is no `RegisterCreateLink` arm on this path), a credentialed validator running modified code — writing DHT ops directly, bypassing the coordinator's Guard 3 — could publish N links and trip the gate with one real participant, opening the reveal phase prematurely and locking out honest validators (whose commits are then rejected by the late-commit guard). **No outcome is forged**: the governance completeness gate and the `HarmonyRecord` duplicate-validator dedup still hold. Blast radius is liveness only, insider-only, recoverable via `force_finalize_round`. Fixed: the gate now counts distinct `link.author` (the signed, unforgeable committer pubkey); `get_attestations_for_request` deduped by author for the same reason. **Coordinator-only — no DNA-hash change** (deployable via `UpdateCoordinators`). NB: an external report framed this as a *regression* of a prior "HashSet dedup" fix in this function. That is incorrect — this function never had author-dedup (`git log -S "HashSet"` confirms); the pre-existing HashSet dedup lives in `governance_integrity` on `HarmonyRecord.participating_validators` (a different guard against badge fabrication, still in place — row above). |
| Single committed validator can open the reveal window unilaterally | Independent review (2026-06-10) | Low — liveness (accepted/documented limit) | `PhaseMarker` validation (`attestation_integrity`) verifies only that the marker's author has committed (holds a `CommitmentAnchor` naming them as `validator`) — it **cannot** verify quorum, because `get_links` is non-deterministic inside `validate()`. So any one committed validator can write a `RevealOpen` `PhaseMarker` directly; combined with the late-commit guard this lets a single insider commit-first, open reveal, and stall the round until timeout finalization. Commit-reveal integrity is unaffected — the researcher stays bound to their pre-committed hash, commitments stay confidential, no outcome can be fabricated. The property *"all validators commit before any reveal opens"* is therefore enforced in the happy-path coordinator, **not** at the network layer. Accepted as a documented liveness limit (recoverable via `force_finalize_round`). The distinct-validator quorum gate (row above) raises the bar for the link-inflation path but does not remove this PhaseMarker vector. |
