...
<div class="explainers">
  ...
  <details>
    ...
    <div class="expand-body">
      ...
      <p>Every party — the researcher and each validator — cryptographically hashes and seals their verdict <em>before</em> anyone reveals anything. These commitments are stored on a Holochain DHT: a distributed hash table where no single node is in charge. Once all commitments are on the network, you trigger the reveal phase. Because every sealed hash is already public and <strong>immutable</strong> before any answer is revealed, an answer changed after the fact can never match its prior commitment — and on production ValiChord networks the conductor enforces that match on-chain at reveal time. No last-mover advantage. No institutional pressure that could nudge a result retroactively.</p>
